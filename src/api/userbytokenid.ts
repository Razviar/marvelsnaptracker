...
export async function tokencheck(request: string): Promise<TokenCheckRes | undefined> {
  return parseTokenCheckRes(
    await Request.post<{request: string}>(
      `/snap/donew2.php?cmd=cm_tokencheck&version=${app.getVersion()}${isMac() ? 'm' : 'w'}`,
      {
        request,
      }
    )
...
export async function tokenrequest({playerid, plguid}: {playerid: string; plguid: string}): Promise<TokenRequestRes> {
  return parseTokenRequestRes(
    await Request.post(`/snap/donew2.php?cmd=cm_tokenrequest&version=${app.getVersion()}${isMac() ? 'm' : 'w'}`, {
      playerid,
      plguid,
    })
  );
...
export async function userbytokenid(tokenid: string): Promise<UserResult> {
  ...
  return parseUserResult(
    await Request.post<UserRequest>(
      `/snap/donew2.php?cmd=cm_userbytokenid&version=${app.getVersion()}${isMac() ? 'm' : 'w'}`,
      {
